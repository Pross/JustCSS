...
add_action( 'init', 'justcss_add_defaults_fn' );
...
function justcss_options_init_fn(){
	...
	add_settings_section( 'main_section', 'Theme Settings', 'justcss_section_text_fn', __FILE__ );
	add_settings_section( 'colours_section', 'Theme Colours', 'justcss_section_text_blank', __FILE__ );
	add_settings_field( 'justcss_google_fonts', 'Enable Google fonts', 'justcss_setting_font_enable', __FILE__, 'main_section' );
	add_settings_field( 'main_font', 'Google Font', 'justcss_setting_font_dropdown_fn', __FILE__, 'main_section' );
	add_settings_field( 'justcss_nav_col', 'Select menu color scheme', 'justcss_setting_nav_dropdown_fn', __FILE__, 'main_section' );
	add_settings_field( 'justcss_bpo_radio', 'Enable bypostauthor css', 'justcss_setting_bpo_enable_fn', __FILE__, 'main_section' );
	add_settings_field( 'justcss_corners', 'Corner radius', 'justcss_setting_corner_fn', __FILE__, 'main_section' );
	add_settings_field( 'justcss_brackets', 'Enable {} around blog title', 'justcss_setting_brackets_fn', __FILE__, 'main_section' );
	add_settings_field( 'justcss_width', 'Theme width px', 'justcss_setting_width_fn', __FILE__, 'main_section' );
	add_settings_field( 'justcss_nav', 'Navbar Colour', 'justcss_setting_nav_fn', __FILE__, 'colours_section' );
	add_settings_field( 'justcss_widget', 'Widget Colour', 'justcss_setting_widget_fn', __FILE__, 'colours_section' );
	add_settings_field( 'justcss_sticky', 'Sticky Colour', 'justcss_setting_sticky_fn', __FILE__, 'colours_section' );
	add_settings_field( 'justcss_bypostauthor', 'Colour for .bypostauthor', 'justcss_setting_bypostauthor_fn', __FILE__, 'colours_section' );
	add_settings_field( 'justcss_comment_even', 'Comments even', 'justcss_setting_comment_even_fn', __FILE__, 'colours_section' );
	add_settings_field( 'justcss_comment_odd', 'Comments odd', 'justcss_setting_comment_odd_fn', __FILE__, 'colours_section' );
	add_settings_field( 'justcss_aside', 'Aside Colour', 'justcss_setting_aside_fn', __FILE__, 'colours_section' );
	add_settings_field( 'justcss_mainfont', 'Primary font colour', 'justcss_setting_mainfont_fn', __FILE__, 'colours_section' );
	add_settings_field( 'justcss_reset', 'Restore Defaults?', 'justcss_setting_reset_fn', __FILE__, 'colours_section' );
...
function justcss_options_add_page_fn() {
	add_theme_page( 'JustCSS Options', 'JustCSS Options', 'edit_theme_options', basename( __FILE__ ), 'justcss_options_page_fn' );
}

/**
 * Display the admin options page
 */
function justcss_options_page_fn() {
?>
	<div class="wrap">
 ...
 * Callback functions
 */
function  justcss_section_text_fn() {
...
function justcss_section_text_blank() {
	// nothing to see here
}

function justcss_setting_nav_fn() {
...
function justcss_setting_nav_dropdown_fn() {
	$options = get_option('justcss_options');
...
function justcss_setting_font_enable() {
	$options = get_option('justcss_options');
	$check = ( isset( $options['justcss_google_fonts'] ) ) ? 1 : 0;
	echo '<input ' . checked( $check, '1' , false )   . ' value="1" id="enable" name="justcss_options[justcss_google_fonts]" type="checkbox" /> Enable';
}

function justcss_setting_font_dropdown_fn() {
	$options = get_option('justcss_options');
